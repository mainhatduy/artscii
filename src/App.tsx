...
function App() {
  ...
  const [uploaded, setUploaded] = useState<{ file: string; name: string } | null>(null);
  ...
  const select = (index: number) => { setActive(index); setLoop(false); };
  async function upload(file?: File) {
    if (!file) return;
    if (!['image/png', 'image/jpeg', 'image/webp', 'image/svg+xml'].includes(file.type)) { setToast('Please choose a PNG, JPG, WebP, or SVG image.'); return; }
    if (file.size > 10 * 1024 * 1024) { setToast('Choose an image smaller than 10 MB.'); return; }
    const url = URL.createObjectURL(file);
    try { await sampleImage(url, density); setUploaded({ file: url, name: file.name.replace(/\.[^.]+$/, '') }); select(4); setToast('Your image is ready. Make it your own.'); }
    catch { URL.revokeObjectURL(url); setToast('No usable shape found. Try a visible silhouette on a transparent or white background.'); }
  }
  ...
  return (
    <div className="app-shell">
        ...
        <section className={`workspace ${expanded ? 'expanded' : ''}`} aria-label="ASCII art editor">
          <div className="preview-panel">
            ...
            <div className={`art-stage ${dragging ? 'dragging' : ''}`} onDragOver={e => { e.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={e => { e.preventDefault(); setDragging(false); void upload(e.dataTransfer.files[0]); }}>
              <AsciiMorph ref={art} images={items.map(p => p.file)} activeIndex={active} characters={characters} density={density} morphDuration={duration} colorMode={sourceColor ? 'source' : 'mono'} palette={palette} playing={playing} motion={motion} grain={grain} onCount={setCount} onError={setToast} />
      ...
      {modal && <div className="modal-backdrop" onClick={() => setModal(null)}><section className="modal" role="dialog" aria-modal="true" aria-labelledby="modal-title" onClick={e => e.stopPropagation()}><button autoFocus className="modal-close icon-button" aria-label="Close dialog" onClick={() => setModal(null)}><X size={20} /></button><span className="eyebrow">A LITTLE ART. A LITTLE ALGORITHM.</span><h2 id="modal-title">{modal === 'about' ? 'Hello, character.' : 'Take the motion with you.'}</h2>{modal === 'about' ? <><p>ArtSCII is a small playground for living ASCII art. Each character is a particle that finds its place in a shape, wanders, and flows into the next.</p><p>Pick a shape, tune the details, or upload your own. Transparent silhouettes and images on white backgrounds work best. Your images stay in your browser.</p><div className="about-features"><span><Maximize2 size={17} /> Responsive Canvas</span><span><Shuffle size={17} /> Particle morphing</span><span><ImagePlus size={17} /> Your own images</span></div><p className="fine-print">Motion respects your system’s reduced-motion preference. PNG exports include the artwork and background.</p></> : <><p>Use <code>src/components/AsciiMorph.tsx</code> in your React project, then add your shape images to its public folder.</p><pre>{code}</pre><button className="primary-button" onClick={async () => { try { await navigator.clipboard.writeText(code); setToast('Component snippet copied.'); } catch { setToast('Clipboard unavailable. Select and copy the snippet above.'); } }}>Copy component snippet <Code2 size={16} /></button></>}</section></div>}
    </div>
  );
}
